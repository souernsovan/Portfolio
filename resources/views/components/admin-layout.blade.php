...
<body class="theme-shell flex h-screen overflow-hidden font-sans antialiased">
    @php
        ...
        $navGroups = [
            [
                ...
                'pattern' => ['admin.projects.*', 'admin.skills.*', 'admin.experiences.*', 'admin.testimonials.*'],
            ...
            [
                ...
                'pattern' => ['admin.profile.*', 'admin.users.*'],
    ...
    <div id="sidebar-backdrop" onclick="window.__closeSidebar()" class="fixed inset-0 z-30 hidden bg-black/50 lg:hidden"></div>

    <!-- Sidebar -->
    <aside id="admin-sidebar" class="panel-sidebar fixed inset-y-0 left-0 z-40 flex h-screen w-64 -translate-x-full shrink-0 flex-col overflow-y-auto border-r py-6 transition-transform duration-200 ease-in-out lg:static lg:translate-x-0">
        ...
        <nav class="flex-1 space-y-1 px-3 text-sm">
            @foreach($navItems as $item)
                ...
                <a href="{{ route($item['route']) }}"
                   class="group flex items-center gap-3 rounded-xl px-3 py-2.5 font-medium transition {{ $active ? 'nav-item-active' : 'nav-item' }}">
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="h-5 w-5 shrink-0 {{ $active ? 'text-blue-600 dark:text-blue-300' : 'text-faint' }}">
                        {!! $item['icon'] !!}
                    </svg>
                    <span class="flex-1">{{ $item['label'] }}</span>
                    @if(!empty($item['badge']))
                        <span class="rounded-full px-2 py-0.5 text-xs font-semibold {{ $active ? 'bg-blue-600/15 text-blue-700 dark:bg-blue-300/20 dark:text-blue-200' : 'bg-emerald-500 text-white dark:bg-emerald-400 dark:text-neutral-950' }}">{{ $item['badge'] }}</span>
                    @endif
                </a>
            @endforeach

            @foreach($navGroups as $navGroup)
                @php $groupActive = request()->routeIs(...$navGroup['pattern']); @endphp
                <details class="group/section pt-1" @if($groupActive) open @endif>
                    <summary class="flex cursor-pointer list-none items-center gap-3 rounded-xl px-3 py-2.5 font-medium transition [&::-webkit-details-marker]:hidden {{ $groupActive ? 'nav-item-active' : 'nav-item' }}">
                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="h-5 w-5 shrink-0 {{ $groupActive ? 'text-blue-600 dark:text-blue-300' : 'text-faint' }}">
                            {!! $navGroup['icon'] !!}
                        </svg>
                        <span class="flex-1">{{ $navGroup['label'] }}</span>
                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="text-faint h-4 w-4 shrink-0 transition group-open/section:rotate-180">
                            <path stroke-linecap="round" stroke-linejoin="round" d="m19.5 8.25-7.5 7.5-7.5-7.5" />
                        </svg>
                    </summary>
                    <div class="border-app-subtle mt-1 ml-4 space-y-0.5 border-l pl-3">
                        @foreach($navGroup['items'] as $sub)
                            @php $subActive = request()->routeIs($sub['pattern']); @endphp
                            <a href="{{ route($sub['route']) }}"
                               class="flex items-center gap-2.5 rounded-lg px-3 py-2 transition {{ $subActive ? 'nav-item-active' : 'nav-item' }}">
                                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="h-4 w-4 shrink-0 {{ $subActive ? 'text-blue-600 dark:text-blue-300' : 'text-faint' }}">
                                    {!! $sub['icon'] !!}
                                </svg>
                                {{ $sub['label'] }}
                            </a>
    ...
    <div class="flex h-screen flex-1 flex-col overflow-y-auto">
        <!-- Header -->
        <header class="panel-header sticky top-0 z-20 flex shrink-0 items-center justify-between gap-3 border-b px-4 py-3 backdrop-blur sm:px-8 sm:py-4">
            <div class="flex min-w-0 items-center gap-3">
                <button type="button" onclick="window.__toggleSidebar()" title="Toggle menu" aria-label="Toggle menu"
                        class="nav-item flex h-9 w-9 shrink-0 items-center justify-center rounded-lg transition lg:hidden">
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="h-5 w-5">
                        <path stroke-linecap="round" stroke-linejoin="round" d="M3.75 6.75h16.5M3.75 12h16.5m-16.5 5.25h16.5" />
                    </svg>
                </button>
                <div class="min-w-0">
                    <h1 class="text-heading truncate text-lg font-semibold">{{ $title ?? 'Dashboard' }}</h1>
                    @isset($subtitle)
                        <p class="text-muted hidden truncate text-sm sm:block">{{ $subtitle }}</p>
                    @endisset
                </div>
            </div>
            ...
            <div class="flex shrink-0 items-center gap-1 sm:gap-2">
                ...
                <a href="{{ route('home') }}" target="_blank" title="View site" class="nav-item flex items-center gap-2 rounded-lg px-2 py-2 text-sm font-medium transition sm:px-3">
                ...
                <details class="group relative">
                    ...
                    <div class="panel-dropdown absolute right-0 z-30 mt-2 w-56 rounded-xl border p-2 shadow-2xl">
                        ...
                        <a href="{{ route('admin.users.index') }}" class="nav-item flex items-center gap-2.5 rounded-lg px-3 py-2 text-sm transition">
                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="text-faint h-4 w-4 shrink-0">
        ...
        <div class="flex-1 p-3 sm:p-6">
            <div class="panel-card min-h-[calc(100vh-6.5rem)] w-full rounded-2xl border p-4 sm:p-8">
                @if (session('status'))
    ...
    <script>
        ...
        window.__toggleSidebar = () => {
            document.getElementById('admin-sidebar').classList.contains('-translate-x-full')
                ? window.__openSidebar()
                : window.__closeSidebar();
        };

        document.getElementById('admin-sidebar').querySelectorAll('a').forEach((link) => {
            link.addEventListener('click', () => window.__closeSidebar());
        });

        window.addEventListener('resize', () => {
            if (window.innerWidth >= 1024) window.__closeSidebar();
        });
    </script>
</body>
</html>
